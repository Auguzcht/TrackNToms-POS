...
const ProductCatalog = ({
  ...
  onSelectProduct,
  loading,
  inventory = {} // Add inventory prop with default empty object
}) => {
  ...
  return (
    <Card className="h-full flex flex-col">
      ...
      {/* Products grid with inventory status integrated */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredProducts.length === 0 ? (
              ...
              filteredProducts.map((product, index) => {
                // Check inventory status for this product
                const productInventory = inventory[product.id];
                const inStock = isInStock(product);
                const inventoryQty = productInventory?.quantity || 0;
                const isLowStock = productInventory && inventoryQty <= productInventory.threshold;
                
                return (
                  <motion.div
                    key={product.id}
                    className={`bg-white border ${inStock ? 'border-[#571C1F]/10' : 'border-red-200'} rounded-lg overflow-hidden cursor-pointer transition-all ${!inStock ? 'opacity-60' : ''}`}
                    onClick={() => inStock && onSelectProduct(product)}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: inStock ? 1 : 0.6 }}
                    transition={{ 
                      duration: 0.3, 
                      delay: index * 0.03,
                      ease: "easeOut"
                  ...
                  >
                    <motion.div 
                      className={`relative h-40 ${inStock ? 'bg-[#FFF6F2]' : 'bg-red-50'} flex items-center justify-center overflow-hidden`}
                      whileHover={inStock ? {
                    ...
                    >
                      {product.image ? (
                        <img
                          src={product.image}
                          alt={product.name}
                          className={`object-cover w-full h-full ${!inStock ? 'opacity-50 grayscale' : ''}`}
                        />
                      ) : (
                      ...
                      {!inStock && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-[1px]">
                          <div className="bg-red-100 text-red-700 py-1 px-3 rounded-md font-medium text-xs border border-red-200">
                            Out of Stock
                          </div>
                        </div>
                      )}
